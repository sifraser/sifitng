<script>
	import { reportTypes, activityTypes, metricTypes, periods } from './navigation.ts';

	let reportType = $state(reportTypes[0]);
	let otherReportTypes = $derived(reportTypes.filter(rt => rt !== reportType));

	let activityType = $state(activityTypes[0]);
	let otherActivityTypes = $derived(activityTypes.filter(at => at !== activityType));

	let metricType = $state(metricTypes[0]);
